...
function getWinePromos(){
      global $conn;

  $promosQ = $conn->prepare("SELECT Wine_id, Promotion_id, expDate FROM WinePromotion");
  $promosQ->execute();
  $promos = $promosQ->fetchAll(PDO::FETCH_CLASS, "WinePromotion");
  return $promos;
 }

function getRandWinePromo(){
      global $conn;

  $promosQ = $conn->prepare("SELECT Wine_id, Promotion_id, expDate FROM WinePromotion ORDER BY RAND() LIMIT 1");
  $promosQ->execute();
  $promos = $promosQ->fetchAll(PDO::FETCH_CLASS, "WinePromotion");
  return $promos;
 }
function getPromoWineById($Promotion_id){
      global $conn;

  $promowQ = $conn->prepare("SELECT Wine_id, Category, subCategory, Name, Descr, Price, Purchases FROM Wine WHERE Wine_id = ?");
  $promowQ->execute();
  $promow = $promowQ->fetchAll(PDO::FETCH_CLASS, "Wine");
  return $promow;
 }
 function getRandPromos(){
      global $conn;

  $promosQ = $conn->prepare("SELECT Title, Discount FROM Promotion ORDER BY RAND() LIMIT 1");
  $promosQ->execute();
 ...
 function getPromos(){
  ...
  $promosQ = $conn->prepare("SELECT Promotion_id, Title, Discount FROM Promotion");
  $promosQ->execute();
  $promos2 = $promosQ->fetchAll(PDO::FETCH_CLASS, "Promotion");
  return $promos2;
 }
